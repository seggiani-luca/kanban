...
void send_card(client *cl, card *c) {
  ...
  cmd cm = {.type = HANDLE_CARD,
            .args = {id_str, c->desc,
                     "fornita card da processare, invio "
    ...
    for (int l = 0; l < 3; l++) {
      ...
      for (int i = 0; i < NUM_COLS; i++) {
        ...
        if (c != NULL) {
          switch (l) {
          case 0:
            printf("id: %-11d user: %-10d", c->id, c->user);
            break;
          case 1: {
...
void exec_command(client_id cl_id, const cmd *cm) {
  ...
  cmd arg_err = {.type = ERR, .args = {"troppi pochi argomenti"}};
