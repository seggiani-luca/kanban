...
I principi che si sono voluti seguire sono:
\begin{itemize}
	\item Protocollo di trasmissione livello \textit{application} basato su stringhe ASCII separate da ritorni carrello (\lstinline|\n|), leggibili da uomo, ispirato a SMTP. Queste stringhe vanno a rappresentare \textit{comandi} dell'applicazione;
	\item Comandi fra server e client inviati su TCP, comandi fra client inviati su UDP (con minima affidabilità basata su ritrasmissioni);
...
\begin{itemize}
	\item \lstinline|core/|: implementa la logica vera e propria della kanban;
	\item \lstinline|net/|: il \textit{modulo di rete}, gestisce i socket e li astrae dagli altri moduli. Nel server, realizza il concetto di \lstinline|connection| con un client;
...
L'appartenenza o meno di una card ad una certa colonna viene rappresentata dalla sua presenza in un'apposita tabella di colonne: \lstinline|card *columns[NUM_COLS][MAX_CARDS_PER_COL]|.
...
\end{lstlisting}
	\item Per la ricezione di comandi si può deserializzare usando l'helper \lstinline|buf_to_cmd(char *buf, cmd *cm)|, che si aspetta di trovare un comando vuoto (per essere sicuri della terminazione della lista di argomenti):
\begin{lstlisting}[language=C++]	
...
Questo non è altro che il client di cui si prende il controllo interagendo con la riga di comando offerta dal server stesso, e viene fornito per creare nuove card effettuare operazioni di debugging (e.g. spostare card). 

Si rende poi disponibile una \lstinline|send_client(client_id cl_id, const cmd *cm)|.

A questo punto la gestione dello stato del sistema e della logica di competenza è delegata al modulo \lstinline|core|, che deve solo portare avanti l'operazione richiesta sulla base del tipo di \lstinline|cmd| fornito in \lstinline|handle_command(client_id cl_id, const cmd *cm)| (chiamata da \lstinline|handle_client()|).

Notiamo infine che modulo net e \lstinline|core| del server usano astrazioni diverse per i client.
...
\end{lstlisting}
mentre nel modulo \lstinline|core| si mantiene informazione relativa alla kanban:
\begin{lstlisting}[language=C++]	
typedef struct {
...
Si nota la stretta dipendenza fra il modulo \lstinline|watch| e il modulo core: quest'ultimo fornisce appositamente due header, \lstinline|core.h| e \lstinline|core_watch.h|, di cui il secondo più intimo e riservato al modulo \lstinline|watch|. 
...
Il flag \lstinline|block| permette inoltre di specificare se si vuole che la funzione blocchi fino alla ricezione di un comando diretto a core, o se si vuole uscire dopo un timeout di 1 secondo.
Questo risulta particolarmente utile per l'implementazione dell'attesa casuale.

In questo modo si riescono quindi a gestire le richieste dirette al client nei \textit{"momenti morti"} accennati sopra.
\begin{lstlisting}[language=C++]	
...
L'implementazione del client diventa quindi semplice: basta chiamare la \lstinline|recv_multi()| per il prossimo comando \lstinline|core|, con gli altri comandi gestiti dietro le quinte. Vediamo ad esempio come ci si registra al server:
\begin{lstlisting}[language=C++]	
int hello() {
  ...
  // attendi ack
  return get_ack(); // incapsula recv_multi()
}
\end{lstlisting}
Funzioni di questo tipo (definite nel modulo \lstinline|core| del client) vengono chiamate in un loop che ottiene card, aspetta un tempo casuale, e restituisce la conferma di aver processato la card al server.
