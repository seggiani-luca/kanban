...
L'idea dietro i moduli di rete è che questi separano la logica di lettura e scrittura su socket dai moduli \lstinline|core|, che devono preoccuparsi solo di logica relativa alla lavagna. In particolare, abbiamo che:
\begin{itemize}
...
A questo punto la gestione dello stato del sistemo e della logica di competenza è delegata al modulo \lstinline|core|, che deve sempre portare avanti l'operazione richiesta sulla base del tipo di \lstinline|cmd| fornito in \lstinline|handle_command(client_id cl_id, const cmd *cm)| (chiamata da \lstinline|handle_client()|).
